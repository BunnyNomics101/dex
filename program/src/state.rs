...
pub struct DexState {
    ...
    pub fee_tier_taker_bps_rates: [u64; 6],
    /// Fee tier maker rates 
    pub fee_tier_maker_bps_rebates: [u64; 6],
}
...
impl FeeTier {
    pub fn from_srm_and_msrm_balances(
        ...
        let one_bunny = 1_000_000_000;
        let idx = match dex_state.fee_tier_thresholds
            .binary_search_by_key(&bunny_held, |n| (one_bunny * n))
        {
            Ok(idx) => idx + 1,
            Err(idx) => idx,
        };
    ...
    pub fn get(
        ...
        let bunny_held = match parsed_token_account.mint {
            a if a == BUNNY_MINT => parsed_token_account.amount,
            _ => {
        ...
        Ok(Self::from_srm_and_msrm_balances(
            dex_state, bunny_held,
        ))
    }

    pub fn taker_rate(self) -> u64 {
        match self {
            FeeTier::T0 => (10 << 32) / 10_000,
            FeeTier::T1 => (9 << 32) / 10_000,
            FeeTier::T2 => (6 << 32) / 10_000,
            FeeTier::T3 => (5 << 32) / 10_000,
            FeeTier::T4 => (3 << 32) / 10_000,
            FeeTier::T5 => (1 << 32) / 10_000,
        }
    }

    pub fn maker_rebate(self, quote_qty: u64) -> u64 {
        let rate = (1 << 32) / 10_000;
